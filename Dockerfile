...
RUN cargo chef cook --release --recipe-path recipe.json -p pacman-server

# Now the actual source, which is the only thing that usually changed.
COPY Cargo.toml Cargo.lock* ./
COPY crates ./crates
RUN cargo build --release -p pacman-server \
    && strip target/release/pacman-server
...
RUN useradd --system --uid 10001 --create-home pacman
USER pacman
WORKDIR /home/pacman
...
ENTRYPOINT ["/usr/local/bin/pacman-server"]
